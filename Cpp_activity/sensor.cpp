...
class Sensor {
    ...
    public:
    Sensor():m_name(""),m_id(count){
        count++;
    }
    Sensor(std::string name):m_name(name),m_id(count){
        count++;
    }
    ~Sensor(){
        count--;
...
int main(){
    ...
    std::vector<Sensor*> acc_ptr;
    std::vector<Sensor*> mag_ptr;

    for(auto entry : j_complete){
        Sensor *data_ptr_acc;
        Sensor *data_ptr_magnet;
        Sensor *data_ptr_gps;
        data_ptr_acc  = new Accelerometer(entry["ACCELEROMETER_X"].get<double>(),
                                     entry["ACCELEROMETER_Y"].get<double>(),
                                     entry["ACCELEROMETER_Z"].get<double>());
        acc_ptr.push_back(data_ptr_acc);
        data_ptr_magnet  = new Magnetometer(entry["MAGNETIC FIELD X (μT)"].get<double>(),
                                     entry["MAGNETIC FIELD_Y"].get<double>(),
                                     entry["MAGNETIC FIELD_Z"].get<double>());
        mag_ptr.push_back(data_ptr_magnet);
        data_ptr_gps  = new GPS(entry["LOCATION_Latitude"].get<std::string>(),
        ...
        std::vector<Sensor *>::iterator iter_1;
        for(iter_1=acc_ptr.begin();iter_1!=acc_ptr.end();++iter_1){
            (*iter_1)->print_raw();
        }
        // std::cout<<<<"\n";
        // std::cout<<typeid(*acc_ptr.at(4)).name()<<"\n";
}   